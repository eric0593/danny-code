...
int process_diag_data(unsigned char *ptr, int len, void *context_data) {
    ...
    ALOGI("Process diag reply data %lx", (long)context_data);

    if(len <= 0) {
        ALOGE("Invalid parameter");
        return -1;
    }
    //printf("process_diag_data %d %d %d\n",ptr[0],ptr[1],ptr[2]);

    cmd_code = ptr[0];
    ALOGI("rsp cmd_code=%d\n", cmd_code);
    if((cmd_code == DIAG_NV_READ_F) || (cmd_code == DIAG_NV_WRITE_F)) {
        ...
        ALOGI("rsp cmd_code=%d, item_id=%d, data_size=%d\n", cmd_code, rspItem.item_id, rspItem.length);
        //printf("rsp cmd_code=%d, item_id=%d, data_size=%d\n", cmd_code, rspItem.item_id, rspItem.length);
        sem_post(&sem_nv_rw_complete);
        return 0;
    }

    if (cmd_code==DIAG_SUBSYS_CMD_F&&ptr[1]==DIAG_SUBSYS_FS&&ptr[2]==48)//efs_sync
    {
        sem_post(&sem_nv_rw_complete);
        return 0;
    }
    // DIAG_BAD_SPC_MODE_F 66
    if (cmd_code == DIAG_BAD_SPC_MODE_F)
    {
        printf("bad spc\n");
        sem_post(&sem_nv_rw_complete);
    }
...
static void process_reply(unsigned char *buf_ptr, int len) {
    ...
    ALOGI("reply item=%d cmd_code=%d\n", rspItem.item_id,rspItem.data[0]);
...
void register_callback(void) {
    ...
    ALOGI("diag: register callback for handle diag data\n");
