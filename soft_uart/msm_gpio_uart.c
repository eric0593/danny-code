...
static ktime_t timer_rx_start;
static int gpio_tx = 0;
static int gpio_rx = 0;
static int rx_bit_index = -1;
#define GPIO_BASE 911//for MSM8909 
#define DEFAULT_SOFT_UART_BAUDRATE 1200 //tx can support 9600 rx can only support 1200
 ...
 */
int msm_soft_uart_init(const int _gpio_tx, const int _gpio_rx)
{
  bool success = true;
  
  mutex_init(&current_tty_mutex);
  
  printk("msm_soft_uart_init=%d \n ",gpio_rx);

  period = ktime_set(0, 1000000000/DEFAULT_SOFT_UART_BAUDRATE);
  timer_rx_start = ktime_set(0, 0);//msm chipset kernel interupt is not real time, we should start the interupt handle asap.
  ...
  gpio_tx = _gpio_tx + GPIO_BASE;
  gpio_rx = _gpio_rx + GPIO_BASE;
  ...
  gpio_set_debounce(gpio_rx,1000*9/DEFAULT_SOFT_UART_BAUDRATE);
  return success;
}
...
{
  if (baudrate>=DEFAULT_SOFT_UART_BAUDRATE)
    period = ktime_set(0, NSEC_PER_SEC/DEFAULT_SOFT_UART_BAUDRATE);
  else
    period = ktime_set(0, NSEC_PER_SEC/baudrate);
  //gpio_set_debounce(gpio_rx, 1000/4800/2); for MSM remove it
  return 1;
}
...
static irq_handler_t handle_rx_start(unsigned int irq, void* device, struct pt_regs* registers)
{
  if (rx_bit_index == -1)
  {
    hrtimer_start(&timer_rx, timer_rx_start, HRTIMER_MODE_REL);
  }
...
static enum hrtimer_restart handle_tx(struct hrtimer* timer)
{
  ...
  spend_time = (s32)(ktime_get().tv64 - current_time.tv64);
  //printk("bit_index=%d  current_time=%dns period=%d character=%c\n",bit_index,(int)current_time.tv64,(int)period.tv64,character);
...
static enum hrtimer_restart handle_rx(struct hrtimer* timer)
{
  ...
  //printk("rx_bit_index=%d  current_time=%dns period=%d bit_value=%x\n",rx_bit_index,(int)current_time.tv64,(int)period.tv64,bit_value);
