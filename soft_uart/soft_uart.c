 ...
 * @date    23 Oct 2019
 * @version 0.1
 * @brief   Use gpio to simulate uart
*/
...
MODULE_AUTHOR("Danny fn");      ///< The author -- visible when you use modinfo
MODULE_DESCRIPTION("A simple Linux driver for the msm tty driver.");  ///< The description -- see modinfo
MODULE_VERSION("0.1");              ///< The version of the module
 
static int gpio_list[] = {5,4};  
...
static int register_uart_to_minor(void){
    int i=0;
    for (i=0; i < N_PORTS; i++){
        if (!msm_soft_uart_init(gpio_list[i], gpio_list[i+1]))
        {
...
static void soft_uart_close(struct tty_struct* tty, struct file* file)
{
  ...
  int wait_time = 0;
  while ((msm_soft_uart_get_tx_queue_size() > 0)
...
{
  return msm_soft_uart_send_string(buffer, buffer_size);
}
...
{
  return msm_soft_uart_get_tx_queue_room();
}
...
{
  return msm_soft_uart_get_tx_queue_size();
}
...
static void soft_uart_set_termios(struct tty_struct* tty, struct ktermios* termios)
{
  ...
  if (!msm_soft_uart_set_baudrate(baudrate))
  {
