...
static u8 p0_port_be_press_value[X_NUM]; /*根据硬件设计， 每一列按下去P0口的值，通过这个值，可以判断是哪列被按下*/
...
static const unsigned short  key_array[Y_NUM][8] = {
        {KEY_8, KEY_7, KEY_6, KEY_5,KEY_4,KEY_3,KEY_2,KEY_1},
        {KEY_9, KEY_0, KEY_MINUS, KEY_EQUAL,KEY_LEFT,KEY_UP,KEY_DOWN,KEY_RIGHT},
        {KEY_T, KEY_R, 0xff, KEY_P,KEY_O, KEY_I, KEY_U, KEY_Y},// KEY_P
        {KEY_Q, KEY_CAPSLOCK, KEY_TAB, KEY_S,KEY_A,KEY_E,KEY_D,KEY_W},
        {KEY_G, KEY_F, KEY_J, KEY_H,KEY_K,KEY_L,KEY_SEMICOLON,KEY_ENTER},
        {KEY_LEFTSHIFT, KEY_N, KEY_M, KEY_V,KEY_B,KEY_X,KEY_C,KEY_Z},
        {0xff, KEY_LEFTMETA, KEY_LEFTALT, KEY_SPACE,KEY_LEFTCTRL,KEY_LEFT,KEY_DOWN,KEY_RIGHTALT},
        //{KEY_RIGHTSHIFT, KEY_RIGHT, KEY_UP, 0xff,0xff,0xff,0xff,0xff},
};
...
u16 keyboard_get_func_press_key(void)
{
    ...
    p0_value = (aw9523b_get_P0_value()&0x80);
    if (p0_value)
    {
        return 0xFF;
    }
...
{
    u8 p0_index = 0xFF, p1_index = 0XFF;
    u8 i = 0;
    u8 p0_value = 0xff;

    p0_value = (aw9523b_get_P0_value()&p0_kbd_used_mask);

    AW9523_LOG("p0_value=%x\r\n", p0_value);
    ...
    for (i = 0; i < X_NUM; i++) {
        if(p0_value == p0_port_be_press_value[i]){
            p0_index = i;
            break;
        }
    }
    ...
    for (i = 0;i < Y_NUM; i++) {
        ...
        p0_value = (aw9523b_get_P0_value()&p0_kbd_used_mask);
        AW9523_LOG("p0_value=%x \r\n", p0_value);
        if(((p0_value)&(p0_kbd_used_mask))==p0_port_be_press_value[p0_index]) 
        {
            p1_index = i;
            break;
        }
    ...
    if (p0_index != 0xFF && p1_index!= 0xFF ) {
...
normal_key:
    keycode = keyboard_get_press_key();
    AW9523_LOG("keyboard_get_press_key  keycode=%x\r\n",keycode);
