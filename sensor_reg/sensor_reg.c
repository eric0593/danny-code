...
void dump(uint16_t item ,uint8_t *data, uint8_t len)
{
    uint8_t i = 0;
    printf("\ndump %d len %d value\n",item,len);
    for (i=0;i<len;i++)
        printf("%x",data[len-1-i]);
}
...
int main(int argc, char* argv[])
{
    ...
    ret = sensor_reg_read( 1984, SENSOR_REG_TIMEOUT, &rep_1984, &len );
    ...
    //dump(1984,rep_1984,len);

    ret = sensor_reg_read( 1942, SENSOR_REG_TIMEOUT, &rep_1942, &len );
    if (ret != SENSOR1_SUCCESS)
    {
        printf("read failed %d\n",ret);
        return -1;
    }
    //dump(1942,rep_1942,len);

    printf("\Current near [%d] far [%d]\n",(((rep_1984[0]&0xf0)<<4)|rep_1942[1]),(((rep_1984[0]&0x0f)<<8)|rep_1942[0]));    

    if (write)
    {
        write_buf[0] = ((near>>8)<<4)|((far>>8));
        sensor_reg_write(1984,write_buf , 1, SENSOR_REG_TIMEOUT);

        write_buf[0] = (far&0xff);
        write_buf[1] = (near&0xff);         
        sensor_reg_write(1942,write_buf , 2, SENSOR_REG_TIMEOUT);
        
        printf("Update to near [%d] far [%d]\n",near,far);    
    }
    free(rep_1984);
    free(rep_1942);
    return 0;
